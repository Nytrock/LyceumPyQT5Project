...
class Start(QMainWindow):
    ...
    def PressEntry(self):
        ...
        con = sqlite3.connect("db/PyQt5 Project.db")
        cur = con.cursor()
...
class Diary(QMainWindow):
    def __init__(self, name):
        ...
        # Смотрим первое и последнее время выставления оценок, по этому будем регулировать размеры дневника
        con = sqlite3.connect("db/PyQt5 Project.db")
        cur = con.cursor()
        ...
        # От этой переменной будем отталкиваться при заполнении дневника
        ...
        for day in list_days:
            if result[kol]:
                ...
                for i, elem in enumerate(list_2):
                    ...
                    # Некоторые кабинеты называются по-особенному, по типу Лабораторной
...
class TeacherWindow(QMainWindow):
    ...
    # Функция выхода и возвращения на прошлый экран
    def PressExit(self):
        open("Пользователь.txt", "w").close()
        self.ex.show()
        self.hide()
...
class Cabinet(QMainWindow):
    def __init__(self, name, acc):
        super().__init__()
        uic.loadUi("UI/Cabinet.ui", self)
        # В зависимости от того, кто зашёл: учитель или ученик, делаем периферию
        con = sqlite3.connect("db/PyQt5 Project.db")
        cur = con.cursor()
        if acc == "ученик":
            name = list(cur.execute(f"SELECT * FROM Ученики WHERE id='{int(name)}'").fetchone())
...
class ChangeCabinet(QMainWindow):
    def __init__(self, name, acc):
        super().__init__()
        uic.loadUi("UI/ChangeCabinet.ui", self)
        ...
        self.Birthday.setText(name[3])
        self.Age.setText(str(name[4]))
        self.con = sqlite3.connect("db/PyQt5 Project.db")
        cur = self.con.cursor()
        if self.acc == "ученик":
...
class ClassmatesSee(QMainWindow):
    def __init__(self, name):
        # Стили...
        super().__init__()
        uic.loadUi("UI/ListClassmates.ui", self)
        self.Exit.setStyleSheet("background: #f9cc02; border: none; "
                                "color: #ff2a00; border-radius: 10px;")
        self.Title.setStyleSheet("color: #6fc238;")
...
class TableWindow(QMainWindow):
    def __init__(self, name):
        ...
        # Вывод всех оценок, сортированных по дате выставления
        self.Sorting()
    ...
    def Sorting(self):
        con = sqlite3.connect("db/PyQt5 Project.db")
        cur = con.cursor()
        # Находим все оценки для нужного ученика по нужному предмету
...
class MakeMark(QMainWindow):
    def __init__(self, name):
        # Стили
        super().__init__()
        uic.loadUi("UI/MakeMark.ui", self)
        self.setStyleSheet("background-color: #6fc238;")
        for child in self.findChildren(QLabel):
            child.setStyleSheet("QLabel {color: #fff}")
        self.Ok.setStyleSheet("background: #f9cc02; border: none; "
    ...
    # Добавить оценку
    def AddMark(self):
        # Считываем всю информацию и сохраняем
        try:
            con = sqlite3.connect("db/PyQt5 Project.db")
            cur = con.cursor()
...
class Classes(QMainWindow):
    def __init__(self, name):
        # Стили
        super().__init__()
        uic.loadUi("UI/ListClasses.ui", self)
        self.Exit.setStyleSheet("background: #f9cc02; border: none; "
                                "color: #ff2a00; border-radius: 10px;")
        self.Title.setStyleSheet("color: #6fc238;")
        self.table.setStyleSheet("background-color: #eff7e5;")
...
class TimeTableTeacher(QMainWindow):
    def __init__(self, name):
        super().__init__()
        # Стили
        uic.loadUi("UI/TimetableTeacher.ui", self)
        self.ex = TeacherWindow(name[0])
        self.Title.setStyleSheet("color: #6fc238;")
        for child in self.findChildren(QTableWidget):
            child.setStyleSheet("background-color: #eff7e5;")
    ...
    def SaveAll(self):
        ...
        # Подстраховка
            ...
            self.Error.setText("Возникла ошибка. Проверьте правильность заполнения расписания")
            self.Error.setStyleSheet("color: #ff2a00")
...
class CreateClass(QMainWindow):
    def __init__(self, name):
        super().__init__()
        # Стили
        uic.loadUi("UI/NewClass.ui", self)
...
class CreatePupil(QMainWindow):
    def __init__(self, name):
        super().__init__()
        # Стили
        uic.loadUi("UI/NewPupil.ui", self)
        self.ex = Classes(name)
        self.Exit.setStyleSheet("background: #f9cc02; border: none; "
                                "color: #ff2a00; border-radius: 10px;")
        self.Create.setStyleSheet("background: #f9cc02; border: none; "
...
class DeletePupil(QMainWindow):
    def __init__(self, name):
        super().__init__()
        uic.loadUi("UI/DeletePupil.ui", self)
        # Стили
        self.ex = Classes(name)
        self.setStyleSheet("background-color: #6fc238;")
        self.Exit.setStyleSheet("background: #f9cc02; border: none; "
...
class DeleteClass(QMainWindow):
    def __init__(self, name):
        ...
        # Стили
        self.params = {}
        uic.loadUi("UI/DeleteClass.ui", self)
        self.ex = Classes(name)
        self.setStyleSheet("background-color: #6fc238;")
        self.Exit.setStyleSheet("background: #f9cc02; border: none; "
                                "color: #ff2a00; border-radius: 10px;")
